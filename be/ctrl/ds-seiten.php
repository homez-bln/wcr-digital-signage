 ...
 * FIX v6: Kino/SUP/Obstacles hatten falsches Icon 🍔 → korrigiert.
 *         esc_html_or_htmlspecialchars() ans Ende verschoben (PHP-Syntax-Fix).
 */
...
<head>
  ...
  <style>
    .ds-gallery { display:grid; grid-template-columns:repeat(auto-fill,minmax(480px,1fr)); gap:24px; padding:0 0 40px; }
    ...
    .ds-frame-wrap { position:relative; width:100%; background:#f0f2f5; overflow:hidden; }
    .ds-frame-wrap iframe { display:block; position:absolute; top:0; left:0; width:1920px; height:1080px; transform-origin:top left; border:none; opacity:0; transition:opacity .4s; pointer-events:none; }
    ...
    .ds-reload-all { margin-left:auto; }
  </style>
</head>
<body class="bo">
...
<div class="header-controls">
  ...
  <button class="btn-upload ds-reload-all" onclick="dsReloadAll()">↺ Alle neu laden</button>
</div>
...
function dsScale() {
    document.querySelectorAll('.ds-frame-wrap').forEach(function(wrap) {
        ...
        if (!iframe) return;
        var scale = wrap.offsetWidth / 1920;
...
function dsReload(idx) {
    ...
    // onload neu setzen vor src-Änderung
...
function dsInitFrames() {
    document.querySelectorAll('.ds-frame-wrap iframe').forEach(function(frame) {
        ...
        dsStartTimes[idx] = Date.now();
        // Handler VOR dem Setzen der src registrieren
        frame.onload = function() { dsLoaded(idx); };
...
document.addEventListener('DOMContentLoaded', function() {
    dsScale();
    // Kurz warten damit das Layout gerendert ist, dann Iframes laden
