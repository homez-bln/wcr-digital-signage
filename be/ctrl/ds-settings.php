<?php
/**
 * ctrl/ds-settings.php — DS Zentraler Controller v4
 * Theme-Switcher + Farben + Schriftart in einer Seite.
 * BE schreibt Einstellungen über WP REST API (update_option mit vollen WP-Rechten).
 * BE liest Einstellungen direkt aus wp_options per PDO (SELECT-Recht vorhanden).
 */
...
// ── PDO Lesen (SELECT-Recht vorhanden) ───────────────────────
...
function dsc_load_opts(PDO $pdo, array $defaults): array {
    ...
    // WP speichert Arrays als serialize() — wir deserialisieren
...
// ── WP REST API Schreiben ─────────────────────────────────────
// Alle Schreiboperationen laufen über WP's update_option() mit vollen DB-Rechten.
function dsc_wp_post(array $data): array {
    $url     = DSC_WP_API_BASE . '/ds-settings';
    $payload = json_encode(array_merge($data, ['wcr_secret' => DSC_WP_SECRET]), JSON_UNESCAPED_UNICODE);
    $ch      = curl_init($url);
    ...
    return [
        'ok'    => ($code === 200 && isset($json['ok']) && $json['ok'] === true),
        'error' => $err ?: (($code !== 200) ? "HTTP $code: " . substr($body ?: '', 0, 120) : ''),
    ];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'theme') {
        $t = trim($_POST['theme'] ?? '');
        if (isset($THEMES[$t])) {
            $r = dsc_wp_post(['action' => 'theme', 'theme' => $t]);
            if ($r['ok']) {
    ...
    if ($action === 'save') {
        ...
        // Viewport aus bestehenden Werten übernehmen
        ...
        $r = dsc_wp_post(['action' => 'save', 'options' => array_merge($DEFAULTS, $new)]);
        if ($r['ok']) {
...
// Laden — jetzt aus WP-gespeicherten Werten
...
function render() {
  ...
  for (var i=0;i<3;i++) {
    ...
    var cp2 = document.getElementById('pcp-'+i);
    if (cp2) { cp2.style.color=G.white; cp2.style.fontFamily=font; }
  }
